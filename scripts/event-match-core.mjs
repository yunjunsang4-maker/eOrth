...
// INSERT 정책은 행사 마지막 날 18시까지 열려 있고 부스 QR도 살아 있어서, 1일차 매칭을
// 실행한 17:30 이후에도 자정까지 약 6시간 반 동안 지각 제출이 계속 들어온다. 그 한 건이
// 전날 풀에 섞이면 짝 구성이 바뀌어,
...
// 같은 날 타임①→② 이월(slot2Pool, 두 타임 폴백)에는 이 함정이 없다 — `before`는
// `created_at < boundary`라 나중에 행이 늘어날 수 없다. **전날 창만 위쪽이 열려 있었다.**
//
// 그래서 재계산을 버리고, **그날의 마지막 매칭 실행**(하루 1회 체제의 일괄 실행, 두 타임
// 폴백이라면 `--slot 2`)이 **그 순간의 명단을 파일로 확정**해 다음 날이 그걸 그대로 읽는다.
// 파일이 곧 "그때 실제로 보낸 결과"이므로 지각 제출도 exclude 차이도 결과를 바꾸지 못한다.
// (행사 마지막 날은 읽을 다음 날이 없으므로 CLI 가 `--last-day` 로 이 파일 생성을 건너뛴다.)
// ============================================================
 ...
 * meetNow=true는 **행사 당일에 보내는 경우**다(하루 1회 17:30 매칭, 또는 두 타임 폴백의 타임①·②) —
