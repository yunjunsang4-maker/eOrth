...
- 댓글 아이콘: 반드시 공용 `CommentIcon`(`src/components/icons`, 2026-09-20 시안 SVG 21×20 아웃라인, Path "M1.63714 13.5531C…") 사용. 화면 안에서 로컬로 다시 그리지 말 것. 💬 이모지 사용 금지.
- 공유 아이콘: 반드시 공용 `ShareIcon`(`src/components/icons`, 2026-09-20 시안 SVG 22×21 종이비행기 아웃라인, mask 구조) 사용. 화면 안에서 로컬로 다시 그리지 말 것.
