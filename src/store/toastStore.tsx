...
interface ToastContextType {
  queue: ToastItem[];
  // 발생 순서대로 큐 맨 뒤에 추가. visual을 주면 배너에 아바타·아이콘·썸네일이 함께 그려진다
  pushToast: (message: string, onPress?: () => void, visual?: ToastVisual) => void;
...
export function ToastProvider({ children }: { children: React.ReactNode }) {
  ...
  // 큐 상한 — 좋아요가 한꺼번에 몰리면 하나씩 3초씩 돌며 몇십 초가 걸린다.
  // 오래된 것부터 버려 최근 알림이 먼저 보이게 한다.
  const MAX_QUEUE = 4;
  const pushToast = useCallback((message: string, onPress?: () => void, visual?: ToastVisual) => {
    setQueue((q) => [...q, { id: `${Date.now()}-${seq.current++}`, message, onPress, visual }].slice(-MAX_QUEUE));
  }, []);
