...
// 밴드 PNG(그림 1073×433)는 시안(402 기준) 좌표와 1:1 스케일. 시안 크롭은 밴드의 두 번째
...
// = 이음새 없는 무한 마퀴.
//
// BAND_W(1088) = 그림 폭 1073 + 이음매 여백 15. 시안 원본은 프랑스 스트립이 맨 앞,
// 베트남 저널이 맨 끝이라 그림이 캔버스를 꽉 채운다(여백 0). 그대로 두 장을 이어붙이면
// 이음매에서만 끝(베트남)과 시작(프랑스)이 딱 붙어 다른 간격들과 어긋나 보인다.
// 그래서 PNG 우측에 45px(3x = 15단위) 투명 여백을 덧대고 BAND_W도 같이 15 늘렸다.
// 두 값은 반드시 함께 움직인다 — 어긋나면 마퀴 리셋 지점에 이음새가 드러난다.
// (BAND_LEFT는 손대지 않는다. 두 번째 장의 원점은 (BAND_LEFT - BAND_W) + BAND_W = BAND_LEFT
//  로 BAND_W와 무관해, 폭을 늘려도 시작 구도가 그대로 유지된다.)
