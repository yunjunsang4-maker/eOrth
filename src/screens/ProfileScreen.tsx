...
export default function ProfileScreen({ navigation, route, pushed, onBack }: ProfileScreenProps) {
  ...
  // 당겨서 새로고침 — 서버에만 있는 것을 여기서 전부 끌어온다:
  //   · 메이트 수 + 메이트 목록
  //   · 내 글 좋아요·댓글 수 (남이 남긴 반응은 로컬에 안 들어온다)
  //   · 다른 기기(아이폰↔안드로이드)에서 쓴 내 글 (syncMyRecords) — 피드는 내 글을 제외하고
  //     계정 전환 pull은 재진입 때 돌지 않아, 여기가 사용자가 직접 당길 수 있는 경로다.
  //     사용자 의도이므로 포그라운드 복귀의 60초 throttle을 우회한다(발행 중 가드는 유효).
  const [refreshing, setRefreshing] = useState(false);
  const onRefresh = useCallback(async () => {
    setRefreshing(true);
    try { await Promise.all([loadNeighborCount(), refreshNeighbors(), refreshMyPostCounts(), syncMyRecords()]); }
    finally { if (followerAliveRef.current) setRefreshing(false); }
  }, [loadNeighborCount, refreshNeighbors, refreshMyPostCounts, syncMyRecords]);
