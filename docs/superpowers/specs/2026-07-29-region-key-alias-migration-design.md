...
- 26개국 dissolve 후 **706개 지역**(코드 기준 병합). 이름 기준으로 병합하면 705개가
  되는데, 아랍에미리트의 동명 지역 2개가 잘못 합쳐진 결과다(1절 「병합은 코드 기준」 참고).
...
보정 후 26개국 **706개 코드가 전부 유일**함을 확인했다.
...
`importSettingsBackup`도 같은 변환기를 통과시킨다. 옛 백업 JSON이 들어올 수 있는데,
...
2. **신 키 유일성** — 변환 결과 706개 코드가 충돌하지 않는지.
