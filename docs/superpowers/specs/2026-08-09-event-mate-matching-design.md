...
| 연락 수단 | **인스타그램 아이디만** 받는다 | 채널 하나로 고정해 발송·검증이 단순해짐. 전화번호를 안 받으니 개인정보 부담도 준다 |
| 결과 전달 | 인스타 DM으로 **수동 발송** | 공식 API는 먼저 말 건 사용자에게만 보낼 수 있어 자동 발송이 불가능하다 |
...
- 문자(SMS) 채널 — 아예 받지 않는다. 자동 발송을 붙이려면 발신번호 사전등록·유료 결제·
  광고성 문자 규제 검토가 필요하고, 수동으로 보낼 거라면 채널이 둘일 이유가 없다.
  **인스타 계정이 없는 사람은 참여할 수 없다** — 부스 안내와 인트로 화면에 미리 알린다.
...
-- 같은 아이디의 중복 제출을 DB에서 막는다. 두 행이 들어가면 그 사람이 두 명으로 매칭되고,
-- 짝 중 한쪽은 이미 임자가 있는 사람을 받는다.
create unique index event_participants_uniq
...
create policy event_participants_insert on event_participants
  ...
  with check (
    ...
    and instagram ~ '^[a-z0-9._]{1,30}$'   -- 인스타 아이디 규칙(소문자 정규화 후)
...
3. **동점은 제출 순서(`created_at`, `id`)로 결정** — 같은 데이터에 항상 같은 결과가 나와야 한다.
   두 번 돌렸는데 짝이 바뀌면 이미 보낸 DM과 어긋난다.
...
- **사람별 발송 문구 + 복사 버튼**, 그리고 그 사람의 DM 창을 여는
  `https://instagram.com/{아이디}` 링크. 전부 수동 발송이므로 **보낸 사람을 체크해 두는
  체크박스**를 함께 둔다(브라우저를 새로 고쳐도 유지되게 `localStorage`). 수십 명을 손으로
  보내다 보면 어디까지 보냈는지가 반드시 헷갈린다.
- **경고 블록** — 미매칭 인원과 사유, 아이디 형식이 수상한 행, 이름 누락 등
...
| 항목 | 내용 |
|---|---|
| 수집 항목 | 이름, 성별, **인스타그램 아이디**, 가고 싶은 나라, 설문 응답 |
| 목적 | 행사 참가자 간 여행 성향 매칭 및 결과 발송 |
| 제공 대상 | **매칭된 참가자 본인** (제공 항목: 이름·인스타그램 아이디·유형 라벨·**겹친 희망 국가**) |
| 보관 기간 | **행사 종료 후 30일**, 이후 파기 |
| 거부 권리 | 동의하지 않으면 참여 불가(매칭이 서비스의 전부이므로) |
| 국외 이전 | Supabase 미국 리전 저장 — 앱 개인정보처리방침과 동일 |
| 처리자 | eOrth (사업자등록번호 171-06-03581) — `docs/privacy-policy.md`와 동일 |
| 삭제 요청 | eorth26@gmail.com — 행사 후에도 언제든 삭제 요청 가능 |
...
③ 정보 입력   이름 · 성별 · 매칭 선호 · 인스타 아이디 · 가고 싶은 나라(1~3개)
...
연락 수단이 하나뿐이라 **아이디를 한 글자 틀리면 그 사람은 결과를 영영 못 받는다.** 전화번호처럼
2차 수단이 없으므로 입력 단계에서 막는다.

- **정규화** — 앞뒤 공백 제거, 맨 앞 `@` 제거, `instagram.com/xxx` 형태로 붙여넣으면 아이디만
  추출, 소문자로 변환. 이 정규화 결과를 저장한다(대문자로 적어도 DM은 같은 계정으로 간다).
- **형식 검사** — `^[a-z0-9._]{1,30}$`. 한글이 섞이면 그 자리에서 막는다(실제로 자주 나온다).
- **확인 단계** — 제출 직전 `@{입력한 아이디}`를 크게 보여주고 "이 계정으로 결과를 보냅니다"를
  확인시킨다. 눈으로 한 번 더 보게 하는 것이 서버 검증보다 오타를 많이 잡는다.
- **중복 처리** — 유니크 인덱스 위반(같은 아이디 재제출)은 오류가 아니라 "이미 참여하셨어요"
  화면으로 받는다. 부스에서 친구 폰으로 다시 시도하는 상황이 실제로 생긴다.
