...
## 6. 개인정보처리방침·앱 공지 — 8/19 개정분과 합친다

직전 개정(공고 2026-08-12 / 시행 2026-08-19, 메이트 추천 활용 명시 + 거부권)이 **아직 시행 전**이다.
따로 두 번 개정하지 않고 **하나로 합친다.**

- **공고일 2026-08-13 / 시행일 2026-08-20** — 새로 추가되는 내용(수집 항목 삭제)에도 제13조의
  7일 전 고지를 지키기 위해 하루 미룬다.
- 제1장 2) 프로필: 수집 항목에서 **생년월일·성별 삭제**
- 제11조: "가입 시 만 14세 이상임을 확인받으며, **생년월일은 수집하지 않는다**"로 개정
- 개정 이력: `2026-08-19` 항목을 `2026-08-20` 으로 바꾸고, 메이트 추천 명시분과 수집 항목
  삭제분을 **한 항목에** 함께 적는다
- md·ko html·en html 3본 동일 반영
- `docs/notices.json`: 기존 `2026-08-19-privacy` 항목을 **삭제하고 `2026-08-20-privacy` 로 신설**한다.
  평소 규칙은 "id를 바꾸지 말 것"(이미 읽은 사용자에게 다시 미읽음으로 뜬다)이지만, 이번엔 공지
  **내용이 실질적으로 바뀌므로 다시 읽혀야 한다.** 기존 공지는 게시 하루째라 혼선이 적다.
- `npm run pages:publish` 로 게시하고 `docs/.published.json` 지문을 커밋한다.

이번 개정은 **수집 항목이 줄어드는** 변경이라 이용자에게 불리하지 않다.

## 6-1. App Store Connect — 방침 URL은 자동, 데이터 수집 신고는 수동

- **개인정보처리방침 URL**: ASC에 등록된 주소가 그대로이므로 게시하면 **자동 반영**된다
  (`https://yunjunsang4-maker.github.io/eOrth/privacy-policy.html`).
- **앱 개인정보(App Privacy) 데이터 수집 신고**: **자동으로 바뀌지 않는다.** ASC > 앱 > 앱 개인정보
  에서 수집 항목을 직접 수정해야 한다. 이번 변경으로 다음 두 항목을 **수집 안 함으로 내려야 한다**:
  - `Sensitive Info` 또는 `Other Data` 로 신고돼 있을 **성별**
  - `Contact Info` 하위의 **생년월일/기타 신원 정보**

  실제 신고 항목명은 콘솔에서 확인해 맞춘다. **방침 문서와 ASC 신고가 어긋나면 같은 5.1.1 조항으로
  다시 거절될 수 있으므로**, 재제출 전에 반드시 맞춘다.
- 재제출 시 **심사 메모(Review Notes)** 에 "Guideline 5.1.1(v) 지적에 따라 성별·생년월일 수집을
  완전히 제거했고, 연령 확인 체크박스로 대체했다"를 적으면 재심사가 빨라진다.
